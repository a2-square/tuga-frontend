 ...
 */
"use strict";
angular.module("angular-toasty", []), angular.module("angular-toasty").directive("toasty", ["toasty", "$timeout", "$sce", function(A, P, g) {
    return {
        replace: !0,
        restrict: "EA",
        scope: !0,
        link: function(A, D, j) {
            function B(P) {
                P ? angular.forEach(A.toasty, function(g, D) {
                    g.id == P && (A.$broadcast("toasty-cleared", A.toasty[D]), g.onRemove && angular.isFunction(g.onRemove) && g.onRemove.call(A.toasty[D]), A.toasty.splice(D, 1), A.$$phase || A.$digest())
                }) : (angular.forEach(A.toasty, function(P, g) {
                    P.onRemove && angular.isFunction(P.onRemove) && P.onRemove.call(A.toasty[g])
                }), A.toasty = [], A.$broadcast("toasty-cleared"))
            }

            function o(A, g) {
                A.timeout = P(function() {
                    B(A.id)
                }, g)
            }

            function w(A, P, g) {
                return 0 == P[g] ? !1 : P[g] ? !0 : A[g]
            }
            ...
            function I(P, D) {
                ...
                var j, B = w(P, D, "sound"),
                    I = w(P, D, "showClose"),
                    Y = w(P, D, "clickToClose"),
                    t = w(P, D, "html"),
                    a = w(P, D, "shake");
                j = D.theme && C.indexOf(D.theme) > -1 ? D.theme : P.theme, A.toasty.length >= P.limit && A.toasty.shift(), B && document.getElementById("toasty-sound").play();
                var s = {
                    id: Q,
                    title: t ? g.trustAsHtml(D.title) : D.title,
                    msg: t ? g.trustAsHtml(D.msg) : D.msg,
                    showClose: I,
                    clickToClose: Y,
                    sound: B,
                    shake: a ? "toasty-shake" : "",
                    html: t,
                    type: "toasty-type-" + D.type,
                    theme: "toasty-theme-" + j,
                    onAdd: D.onAdd && angular.isFunction(D.onAdd) ? D.onAdd : null,
                    onRemove: D.onRemove && angular.isFunction(D.onRemove) ? D.onRemove : null,
                    onClick: D.onClick && angular.isFunction(D.onClick) ? D.onClick : null
                };
                A.toasty.push(s), D.onAdd && angular.isFunction(D.onAdd) && D.onAdd.call(s), A.$broadcast("toasty-added", s), 0 != D.timeout && (D.timeout || P.timeout) && o(A.toasty[A.toasty.length - 1], D.timeout || P.timeout)
            }
            var Q = 0,
                C = ["default", "material", "bootstrap"];
            A.position = "", A.toasty = [], A.$on("toasty-new", function(P, g) {
                var D = g.config,
                    j = g.options;
                A.position || (A.position = "toasty-position-" + D.position), I(D, j)
            }), A.$on("toasty-clear", function(A, P) {
                B(P.id)
            }), A.close = function(A) {
                B(A)
            }, A.clickToasty = function(P) {
                A.$broadcast("toasty-clicked", P), P.onClick && angular.isFunction(P.onClick) && P.onClick.call(P), P.clickToClose && B(P.id)
            }
        },
    ...
    return {
        ...
        $get: function() {
            return {
                config: angular.extend(A, P)
            }
        }
    }
}), angular.module("angular-toasty").factory("toasty", ["$rootScope", "toastyConfig", function(A, P) {
    var g = P.config,
        D = function(P, D) {
            (angular.isString(P) && "" != P || angular.isNumber(P)) && (P = {
                title: P.toString()
            }), P && (P.title || P.msg) ? (P.type = D || "default", A.$broadcast("toasty-new", {
                config: g,
                options: P
            })) : console.error("angular-toasty: No toast title or message specified!")
        };
    return D["default"] = function(A) {
        D(A)
    }, D.info = function(A) {
        D(A, "info")
    }, D.wait = function(A) {
        ...
        A.$broadcast("toasty-clear", {
            id: P
        })
    }, D.getGlobalConfig = function() {
        return g
    }, D
}]);